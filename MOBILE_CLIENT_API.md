...
| Môi trường | Base URL |
|---|---|
| Test (máy vast.ai hiện tại) | `http://70.9.106.136:51074` |
...
> Port `51074` là port public mà vast.ai map cho container port `10100`. Nếu instance
...
```
GET http://70.9.106.136:51074/v1/wardrobe/items?token=<EDGE_TOKEN>
  ...
  "items": [
    {
      ...
      "uploadUrl": "http://70.9.106.136:51147/wardrobe-raw/raw/...?X-Amz-Algorithm=...",
...
```http
PUT http://70.9.106.136:51147/wardrobe-raw/raw/...?X-Amz-Algorithm=...
Content-Type: image/jpeg
Cookie: C.50551515_auth_token=<EDGE_TOKEN>
...
3. **Edge token ở môi trường Test phải đi bằng Cookie, không phải query param** — thêm
   ...
   trên instance hiện tại là `C.50551515_auth_token`. Production không cần cookie này.
...
**Nhịp polling khuyến nghị:** 5–10s. Thời gian đo thực tế trên instance test (RTX 3090 Ti
24GB, toàn bộ model chạy GPU): **~29 giây mỗi ảnh**, gần như toàn bộ nằm ở bước D1. Lô 9
ảnh mất 4 phút 22 giây. Đừng polling dày hơn 5s — không có kết quả sớm hơn, chỉ tốn pin.
...
`processedItems=1`, `failedItems=2`, status `cancelling` → `cancelled` sau khoảng 40s.
...
```jsonc
{
  "items": [
    {
      "imageUrl": "http://70.9.106.136:51147/wardrobe-items/items/...?X-Amz-Algorithm=...",
      "jobId": "e1f0b194-2110-4278-8462-1d6e5e9bee48",
      "tags": {
        "type": "shirts",
        "category": "clothing",
...
từng chi tiết sản phẩm. Ảnh luôn là **hình vuông**, nền trắng, an toàn để hiển thị thẳng
trong lưới mà không cần crop thêm. Thỉnh thoảng ở mép ảnh còn sót một dải nhỏ của món kế
bên khi hai món nằm sát nhau trong lưới — không ảnh hưởng tag, nhưng nếu muốn sạch tuyệt
đối thì hiển thị với `BoxFit.contain` kèm chút padding.
...
```bash
BASE=http://70.9.106.136:51074
EDGE=<EDGE_TOKEN>
APP=$(grep TEST_BEARER_TOKEN ${WORKSPACE:-/workspace}/.env | cut -d= -f2)
COOKIE="C.50551515_auth_token=$EDGE"
