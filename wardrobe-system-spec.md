...
**D1 — bước crop: tách vật thể rồi dựng lại nền.** Mỗi món được **cắt ra theo chính mặt nạ phân vùng của nó**, rồi **ghép lên một nền mới dựng lại** — không phải copy một hình chữ nhật từ ảnh lưới. Bảy điểm đã phải xử lý:

- **Không copy thẳng vùng chữ nhật.** Box được sinh vừa khít món đồ, nhưng hình vuông bao quanh nó thì rộng hơn món đồ mỗi khi trang phục cao — nên vùng copy thò sang ô bên cạnh và **mang theo pixel của món khác**. Đo trên ảnh thật: quần dài có **8.337 px của chiếc áo bên cạnh** nằm trong khung vuông của nó; trên `result_v4.png`, hai chiếc dép cách nhau 11 px khiến mỗi crop chứa **cả đôi**, biến một đôi dép thành hai item trùng nhau trong tủ đồ. D3 sau đó gắn tag, và D2 lấy vân embedding, trên đúng phần lẫn vào đó.
- **Ghép mềm, không dùng khuôn cứng.** Mặt nạ được giữ ở dạng **alpha liên tục** (không nhị phân hóa) rồi alpha-blend, nên viền món đồ giữ được độ chuyển mượt mà model sinh ra thay vì bị răng cưa. Vùng nào không thuộc món đó — kể cả món hàng xóm đứng trong cùng khung vuông — đều bị thay bằng nền mới.
- **Nền dựng lại lấy từ chính màu nền của ảnh lưới**, vì nền là gần-trắng chứ không phải `#ffffff` (ảnh mẫu đo được `(245, 245, 244)`); giữ đúng phông studio phẳng mà D3 được huấn luyện trên đó. **Không** chuyển sang nền trong suốt — D3 sẽ làm phẳng alpha thành đen.
- **Một *món* không phải lúc nào cũng là một vùng liên thông**: đôi dép là hai vùng, túi có quai cuộn bên cạnh cũng có thể là hai. Khoảng cách thuần túy **không** phân biệt được: đo được hai chiếc dép cách nhau 11 px (cần gộp) trong khi ở ảnh khác áo cách quần cũng 11 px (không được gộp). Nên ngoài ngưỡng khoảng cách còn có luật thứ hai: chỉ gộp khi hai vùng **nằm cùng hàng, chồng nhau gần hết theo chiều dọc, và xấp xỉ bằng nhau về cả rộng lẫn cao** — tức là một *đôi* gồm hai vật gần như giống hệt, soi gương; áo với quần thì khác hẳn cả kích thước lẫn hình dáng.
- **Vùng quá nhỏ không bị vứt đi**: quai rời, khóa, đầu dây rút là bộ phận thật của món đồ. Vùng chưa được nhận sẽ được gắn vào món có box chứa ít nhất **80%** diện tích của nó — cùng luật 80% mà D0b dùng để quyết định đồ mang theo thuộc về chủ thể.
...
Mặt nạ do `ZhengPeng7/BiRefNet_lite` sinh (tách nền class-agnostic). Hai cách tiếp cận trước đều thua khi đo trên cùng 9 ảnh sinh thật: **hiệu ảnh nền + thành phần liên thông** gộp nhầm hai món thành một khối mỗi khi viền chúng chạm nhau; **dùng lại detector thời trang của D0b** chỉ đạt 5/9 vì nó phải *phân loại* mới phát hiện được, mà độ tin cậy của nó hiệu chỉnh rất tệ trên miền ảnh mockup tổng hợp này (đôi dép giống hệt nhau về thị giác đạt 0.96 ở lần sinh này và 0.04 ở lần sinh khác). BiRefNet đạt 9/9 ở 100ms/ảnh trên GPU và **không cần ngưỡng điểm nào cả**.
