...
**D0b — `dress` và `top`/`bottom` loại trừ nhau, phải chốt một bên.** `resolve_dress_conflict()` trong `detect_clothing_yolo.py` phân xử bằng cách so hue của vùng trên và vùng dưới, nhưng nó **chỉ chạy khi có đủ cả ba** `dress` + `top` + `bottom`, thiếu một cái là thoát ngay và giữ nguyên. Ảnh selfie chụp **từ ngực trở lên** thì không có `bottom` để nhận, nên một cái áo sơ mi hoạ tiết vừa ăn điểm `top=0.592` vừa ăn điểm `dress=0.515` làm cả hai cờ cùng bật — prompt xin **đồng thời** một cái đầm và một cái áo, và tủ đồ của một người đàn ông nhận về item `mid length dresses`.
...
**D0b — đã tắt đường cứu hộ riêng cho `bag`.** Trước đây túi đè lên vùng áo được chấp nhận ở sàn rất thấp (0.15) thay vì ngưỡng chính, vì túi đeo chéo ôm sát áo thật sự ăn điểm thấp. Nguyên tắc đúng, nhưng **đo trên 60 ảnh thật thì giá quá đắt**: trong dải 0.15–0.30 có 8 ảnh được cứu, gán nhãn bằng mắt ra **7 sai / 1 đúng** — bó hoa, vòng tay, thắt lưng, và những ảnh không hề có túi. Và cái đúng duy nhất (túi đeo chéo thật) lại ở điểm **thấp nhất 0.1518**, nằm dưới cả 7 cái sai (0.1633–0.2969), nên **không ngưỡng nào tách được hai nhóm**.

Một cái túi giả không chỉ thêm một món rác vào tủ. Ở ảnh 766, nó khiến D1 vẽ một **bó hoa** vào ô "bag"; bó hoa đó **chạm vào cái áo**, BiRefNet gộp thành một vùng liên thông (nâng ngưỡng mask lên 0.8 vẫn dính) nên ảnh cái áo trong tủ **dính luôn bó hoa** — tức là hỏng cả món đúng. Sau khi tắt, ảnh đó ra đúng một `t-shirts` sạch.
...
Điều kiện cứu phải chặt thêm một vế: **box đang thắng phải chứa thêm khuôn mặt của người khác**. Không có vế này, bước cứu còn thay cả box đúng của chủ thể bằng một box điểm thấp là **mảnh vỡ của chính người đó**: ảnh 0646, box đúng `0.969` (chỉ chứa mỗi khuôn mặt chủ thể) bị thay bằng box `0.350` cụt vai và thân dưới, SAM chỉ lấy được mặt và bàn tay, và D1 vẽ ra một cái đầu người làm "áo". Box chỉ chứa khuôn mặt chủ thể thì đã là của chủ thể, không có gì để cứu. Đo trên 10 ảnh test: chỉ ảnh 0646 đổi (nay ra đúng áo len sọc), ảnh gốc 9668 vẫn được cứu y như cũ.
...
- Một *món* không phải lúc nào cũng là một vùng liên thông: đôi dép là hai vùng, túi có quai cuộn bên cạnh cũng có thể là hai. **Không có ngưỡng khoảng cách nào tách được việc này** — đo trên grid thật: khoảng cách *trong cùng một món* chạy từ 3px (đôi dép) tới **18px** (túi với sợi dây cuộn cạnh nó), còn *giữa hai món khác nhau* chạy 11px, 15px, 16px ở các bố cục sát nhau. Hai nhóm chồng lấn, và **sợi dây của chính cái túi còn nằm xa hơn cái áo với cái quần bên cạnh**. Đo theo bounding box càng tệ hơn: hai món xếp chéo thì box chồng nhau, khe ra `0px`.
- Thứ tách được là **số món prompt yêu cầu, dùng làm ngân sách dừng**: gộp cặp *gần nhất*, và chỉ gộp tiếp chừng nào số vùng còn **nhiều hơn** số món đã xin. Vẫn có **trần khoảng cách 3%**, nên khi model vẽ thừa một món thì việc gộp dừng lại thay vì hàn hai món thật vào nhau — đó đúng là lỗi của phiên bản trước, bản ép số món bằng cách gán vào ô lưới và đã hàn cái áo vào cái chân váy. Khác biệt cốt lõi: số món là **giới hạn dừng**, không phải **đích phải đạt**. Trên 60 lần sinh của 3 job thật, số grid có số vùng khớp số món tăng **45 → 52**, không grid nào tệ đi.
